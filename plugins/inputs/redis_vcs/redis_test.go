...
func TestRedis_ParseMetrics(t *testing.T) {
	...
	err := gatherInfoOutputOS(rdr, &acc, tags)
	...
	expect_queue_fields := map[string]interface{}{
		"num_message": int64(2),
	}

	expect_queue_tags := map[string]string{
		"host": "redis.net",
		"queue_name": "input_key",
		"DB": "1",
		"server": "localhost",
	}

	dberr := gatherInfoOutputDB(info, &acc, expect_queue_tags)
	...
	acc.AssertContainsTaggedFields(t, "redis_queuespace", expect_queue_fields, expect_queue_tags)
}
